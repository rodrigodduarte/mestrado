...
from model import CustomFeaturesOnlyModelDropIn
from kf_data import CustomFeaturesFromFoldersModule_kf
...
def _build_model(hparams, class_weights):
    ...
    try:
        model = CustomFeaturesOnlyModelDropIn(**model_args, class_weights=class_weights)
        return model, True  # passou via ctor
    except TypeError:
        model = CustomFeaturesOnlyModelDropIn(**model_args)
        injected = False
        try:
