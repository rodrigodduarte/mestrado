...
# train_ensemble_csv_kf.py — CustomEnsembleModel + CustomImageCSVModule_kf (SEM balanceamento)
# - Sempre usa a versão *_kf do DataModule (com K-Fold).
# - Se K_FOLDS > 1: roda K-Fold normal.
# - Se K_FOLDS == 1: emula split (~80/20) com n_splits=5 e executa apenas fold_idx=0.
# - Detecta automaticamente o features_dim a partir de um batch do DataModule.
...
from dataset import CustomImageCSVModule_kf  # SEMPRE usamos a versão _kf
...
def _safe_len_dataset_from_loader(dl):
    """Conta amostras de forma robusta para calcular throughput."""
    try:
        return len(dl.dataset)
    except Exception:
        n = 0
        for b in dl:
            # batch pode ser (image, features, label)
...
def _infer_features_dim(dm):
    """Pega um batch e retorna a dimensão do vetor de características."""
    # usa train_loader após setup('fit')
    train_loader = dm.train_dataloader()
    batch = next(iter(train_loader))
    # batch: (images, features, labels)
    if not isinstance(batch, (list, tuple)) or len(batch) < 2:
        raise RuntimeError("Batch inesperado: não contém 'features'.")
    features = batch[1]
    if isinstance(features, (list, tuple)):
        # concatena/achata se necessário
        features = torch.as_tensor(features[0])
    if features.ndim == 1:
        return int(features.shape[0])
    return int(features.shape[-1])

# ------------- construção do DataModule -------------
def build_datamodule_kf(h, n_splits: int, fold_idx: int):
    """Sempre usa CustomImageCSVModule_kf sem balanceamento (apenas shuffle no treino)."""
    return CustomImageCSVModule_kf(
...
def main():
    ...
    # - Se K_FOLDS == 1: emular split usando n_splits=5 e rodar só fold_idx=0
    if k_splits_cfg > 1:
        effective_n_splits = k_splits_cfg
        folds_to_run = list(range(effective_n_splits))
    else:
    ...
    for seed in range(42, 42 + n_seeds):
        ...
        for fold_idx in folds_to_run:
            ...
            # ----- Inferir features_dim dinamicamente -----
            features_dim = _infer_features_dim(dm)
            print(f"[Info] features_dim detectado: {features_dim}")
            ...
            n_test = _safe_len_dataset_from_loader(test_dl)
