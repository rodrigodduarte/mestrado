...
# extracao_all.py
#
# Benchmark automatizado para medir custo computacional e qualidade de todos
# os modelos profundos em todos os folds, sem salvar checkpoints.
#
# Fluxo:
# - Lê SOMENTE config1.yaml no mesmo diretório deste script.
# - Ajusta TMODEL para cada arquitetura de interesse:
#       convnext_t
#       convnext_t_ne
#       swint_t
#       swint_t_ne
...
#       - treina 1 época
#       - mede tempo de treino, pico de memória
#       - roda validate e test
#       - mede tempo de teste, latência média por imagem e throughput
#       - mede pico de memória no teste
#       - calcula tamanho aproximado do modelo em memória (MB) e #params
# - Salva estatísticas por fold em estatisticas/<dataset>_<modelo>/fold_<k>/
#   (hyperparams_used.json, benchmark_report.json, benchmark_result.txt)
# - Agrega entre folds e gera:
#       estatisticas/<dataset>_<modelo>/resumo.txt
#       estatisticas/<dataset>_tabela_resumo.txt
#
# Observações:
# - Nenhum .ckpt é salvo. Não existe ModelCheckpoint aqui.
# - "model_size_mb" estima o tamanho dos pesos em MB na memória.
# - "epoch_trained" é sempre 0 porque rodamos só 1 época.
#
# Execução:
#   cd treinamentos
...
# treinamentos/
#   extracao_all.py               <- este arquivo
#   config1.yaml                  <- config base
#   dataset.py
#   model.py
#   callbacks.py (se seus modelos importam isso internamente)
#   estatisticas/                 <- pasta de saída (pode estar vazia)
#
# Saída:
# treinamentos/estatisticas/
#   <dataset>_<tmodel>/
#       fold_0/
...
# importa implementações locais do usuário
from model import (
    CustomModel,
    CustomEnsembleModel,
    CustomModelTriple,
    CustomFeaturesOnlyModel,  # não vamos usar 'vector' agora, mas mantemos import
)

from dataset import (
    CustomImageModule_kf,
    CustomImageCSVModule_kf,
    # CustomFeaturesFromFoldersModule_kf será importado dinamicamente se for usado
)
...
def load_hparams(path_yaml: str):
    """Carrega hiperparâmetros do YAML."""
    with open(path_yaml, "r") as f:
        return yaml.safe_load(f)


def set_seeds(seed: int):
    """Força semente global para reproducibilidade."""
...
def len_dataloader_safe(dl):
    """
    Retorna o número total de amostras do dataloader de teste.
    Isso é usado pra calcular latência média por imagem e throughput.
    Faz fallback manual se dl.dataset não tiver __len__.
    """
    try:
...
def infer_features_dim_from_dm(dm):
    """
    Para modelos que combinam imagem + vetor SSN (ensemble, triple),
    inferimos automaticamente a dimensionalidade do vetor de atributos.
...
def build_datamodule(h, n_splits: int, fold_idx: int):
    """
    Cria o DataModule correto de acordo com TMODEL.

    - convnext_t, swint_t:
        usa só imagem -> CustomImageModule_kf
    - convnext_t_ne, swint_t_ne, triple:
        usa imagem + vetor SSN -> CustomImageCSVModule_kf
    - vector:
        usa só vetor SSN -> CustomFeaturesFromFoldersModule_kf
        (não vamos rodar vector agora, mas a lógica fica pronta)
    """
    tmodel = h["TMODEL"]
    ...
    if tmodel in ["vector"]:
        # NÃO vamos usar agora, mas deixo para referência futura
...
def build_model(h, dm_type, dm=None):
    """
    Cria o LightningModule correto conforme TMODEL.

    - convnext_t, swint_t:
        CustomModel (backbone puro)
    - convnext_t_ne, swint_t_ne:
        CustomEnsembleModel (backbone + vetor SSN concatenado)
    - triple:
        CustomModelTriple (ConvNeXt + Swin + SSN juntos)
    - vector:
        CustomFeaturesOnlyModel (apenas vetor SSN, sem imagem)
        (não vamos usar agora, mas fica pronto)
    """
    tmodel = h["TMODEL"]

    # normaliza optimizer_momentum (às vezes é float único, às vezes lista de dois valores)
    ...
    if tmodel in ["convnext_t", "swint_t"]:
        # backbone puro (ConvNeXt ou Swin)
        return CustomModel(
            tmodel=tmodel,
            **common_kwargs,
        )
    ...
    if tmodel in ["convnext_t_ne", "swint_t_ne"]:
        # backbone + vetor de atributos SSN concatenado
    ...
    if tmodel == "vector":
        # modelo só-vetor (não usado agora)
...
def aggregate_reports(reports, dataset_name, tmodel_name):
    """
    Recebe lista de 'report' (um por fold) e calcula agregados:
    - tempo médio de treino/teste
    - latência média por imagem
    - throughput
    - pico de memória GPU
    - tamanho do modelo em MB
    - nº de parâmetros
    - accuracy média, f1 média, etc.
    """
    ...
    for r in reports:
        ...
        tm = r["test_metrics"]
        # As chaves esperadas do LightningModule no test_step:
    ...
    agg = {
        ...
        "test_error_mean_std": mean_std(err_list),  # 1 - accuracy
...
def write_model_resumo(run_dir, agg):
    """
    Cria estatisticas/<dataset>_<tmodel>/resumo.txt,
    descrevendo médias ± std entre folds daquele modelo.
    """
    ...
    with open(resumo_path, "w") as f:
        ...
        f.write(f"Tamanho do modelo MB (mean ± std): {fmt_pair(agg['model_size_mb_mean_std'])}\n")
...
def write_dataset_table(base_save_dir, dataset_name, dataset_results):
    """
    Gera estatisticas/<dataset>_tabela_resumo.txt com um panorama
    comparando cada modelo do dataset:

    - Accuracy (mean±std)
    - Test error (mean±std)
    - Latência ms/img (mean±std)
    - Tamanho do modelo MB (mean±std)
    """
    ...
    pretty_name = {
        ...
        # "vector":      "SSN (vector)"  # propositalmente fora dessa rodada
    }
    ...
    with open(tabela_path, "w") as f:
        ...
        f.write("Modelo ; Accuracy (mean±std) ; TestError (mean±std) ; Latência ms/img (mean±std) ; Modelo MB (mean±std)\n")
        for tmodel, agg in dataset_results.items():
            # ignorar 'vector' se aparecer por acidente
            if tmodel == "vector":
                continue
            f.write(
                ...
                f"{fmt_pair(agg['model_size_mb_mean_std'])}\n"
            )


# -------------------------------------------------
# núcleo do benchmark de UM modelo em UM fold
# (treina 1 época, não salva checkpoint)
# -------------------------------------------------

def _estimate_model_size_mb(model: torch.nn.Module):
    """
    Estima o tamanho em MB ocupado pelos pesos do modelo,
    baseado no dtype atual dos parâmetros.
    Isso substitui 'checkpoint_size_mb' já que não salvamos .ckpt.
    """
    total_bytes = 0
...
def benchmark_single_fold(h, fold_idx, base_save_dir, seed=42):
    """
    Executa benchmark de UM modelo (h['TMODEL']) em UM fold específico.

    Passos:
    - seta seed
    - cria datamodule do fold
    - instancia o modelo
    - treina por 1 época (sem salvar checkpoint)
    - mede pico de memória GPU no treino
    - roda validate() no modelo treinado
    - roda test() no modelo treinado e mede:
        * tempo total de teste
        * latência média por imagem
        * throughput
        * pico de memória GPU no teste
    - calcula tamanho aproximado do modelo em MB
    - calcula nº de parâmetros treináveis
    - salva relatórios .json e .txt (sem pesos)

    Retorna um dicionário 'report' com todos os campos.
    """
    ...
    # "época treinada": sempre 0, porque rodamos só uma época
    epoch_trained = 0
    ...
    # tamanho aproximado do modelo em memória (MB)
    ...
    # salva estatísticas em JSON e TXT humanos
...
# -------------------------------------------------
# main: roda apenas config1.yaml, SEM incluir 'vector'
# -------------------------------------------------
...
def main():
    ...
    # diretório onde todos os resultados serão guardados
    ...
    # (vector / SSN puro será rodado separadamente)
    TMODELS_TO_TRY = [
        "convnext_t",
        "convnext_t_ne",
        "swint_t",
    ...
    SEED = 42  # fixa a seed pra benchmark reproduzível

    dataset_results = {}  # vai guardar agregados por modelo

    # loop por modelo
    for tmodel_name in TMODELS_TO_TRY:
        print(f"\n--- Dataset {dataset_name} | Modelo {tmodel_name} ---")

        # clona config base e injeta TMODEL
        h_local = dict(h_base)
        ...
        # salva resumo agregado desse modelo
        run_name = f"{dataset_name}_{tmodel_name}"
        run_dir = os.path.join(BASE_SAVE_DIR, run_name)
